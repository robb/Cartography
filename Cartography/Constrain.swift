...
/// - parameter block:   A block that declares the layout for `views`.
///
public func constrain<T: Hashable>(_ views: [T: View], replace group: ConstraintGroup = ConstraintGroup(), @noescape block: (([T : LayoutProxy]) -> ())) -> ConstraintGroup {
    let context = Context()
	let proxies = views.map { (key: $0, value: LayoutProxy(context, $1)) }
